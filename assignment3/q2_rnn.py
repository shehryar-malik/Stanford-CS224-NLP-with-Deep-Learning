...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence) < max_length:
            s = sentence + [zero_vector]*(max_length-sentence_length)
            l = labels +  [zero_label]*(max_length-sentence_length)
            mask = [True]*sentence_length + [False]*(max_length-sentence_length)
            ret.append((s, l, mask))
        else:
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        lookup = tf.nn.embedding_lookup(embedding_matrix, self.input_placeholder)
        embeddings = tf.reshape(lookup, [-1, Config.max_length, Config.n_features * Config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        b_2 = tf.get_variable('b_2', shape=[Config.n_classes], initializer=tf.zeros_initializer())
        dims = tf.stack([tf.shape(x)[0], Config.hidden_size])
        hidden = tf.fill(dims, 0.0)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                o_t, hidden = cell(x[:,time_step,:], hidden)
