...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        gvp = optimizer.compute_gradients(loss)

        if self.config.clip_gradients:
            grad = [g for g, _ in gvp]
            var  = [v for _, v in gvp]
            clipped, self.grad_norm = tf.clip_by_global_norm(grad, self.config.max_grad_norm)
            gvp = zip(clipped, var)
        
        else:
            self.grad_norm = tf.global_norm([g for g, _ in gvp])
        
        train_op = optimizer.apply_gradients(gvp)
